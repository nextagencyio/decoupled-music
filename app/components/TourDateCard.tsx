...
export default function TourDateCard({ item }: TourDateCardProps) {
    ...
    >
      ...
      <div className="p-6">
          {(item as any).venueName && (
            <p className={`text-sm font-bold mb-2 inline-block px-3 py-1 rounded-full ${accent.tag}`}>{(item as any).venueName}</p>
          )}
